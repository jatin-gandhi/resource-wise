...
from app.ai.agents.fuzzy.resolvers.category import CategoryResolver, VectorResolver
...
class HybridFuzzyResolver:
    """Hybrid fuzzy resolver using tiered resolution strategy."""

    def __init__(self, config: AIConfig):
        self.config = config
        self.designation_resolver = DesignationResolver(config)
        self.category_resolver = CategoryResolver(config)
    ...
    async def resolve_fuzzy_terms(self, terms: List[str]) -> Dict[str, List[str]]:
        ...
        Parallel: Designation abbreviations (SSE, PM) + Skill categories (frontend, backend)
        Fallback: Vector similarity for unknown terms
        ...
        for term in terms:
            ...
            elif self.category_resolver.can_handle(term):
                category_terms.append(term)
            else:
                unknown_terms.append(term)

        # Process designation and category terms in parallel
        ...
        if category_terms:
            category_results = await self.category_resolver.resolve_terms(category_terms)
            resolved_terms.update(category_results)
    ...
    def get_resolution_stats(self, terms: List[str], resolved_terms: Dict[str, List[str]]) -> Dict:
        ...
        return {
            ...
            "category_resolved": len(category_resolved),
            ...
            "category_terms": category_resolved,
